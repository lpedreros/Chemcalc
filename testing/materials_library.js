/* ============================================================
   ...
   Reads/writes from 'custom_materials' table in Supabase.
   ============================================================ */
...
async function saveLibraryItem(item) {
  ...
  var payload = {
    ...
    source:       item.affiliate_id ? 'affiliate' : 'custom',
    url:          item.url ? item.url.trim() : null,
    affiliate_id: item.affiliate_id || null
  };
  var { data, error } = await _sb.from('custom_materials').insert(payload).select().single();
...
async function deleteLibraryItem(id) {
  var { error } = await _sb.from('custom_materials').delete().eq('id', id);
...
function renderLibraryTable() {
  ...
  tbody.innerHTML = _matLib.map(item => `
    <tr data-lib-id="${item.id}">
      ...
      <td>
        <select class="est-input lib-edit" data-field="source">
          <option value="custom"    ${item.source==='custom'   ?'selected':''}>Custom</option>
          <option value="affiliate" ${item.source==='affiliate'?'selected':''}>Affiliate</option>
        </select>
      </td>
      ...
      <td><input class="est-input lib-edit" data-field="url" value="${_libEscHtml(item.url || '')}" placeholder="https://..." /></td>
...
async function addLibraryItemFromForm() {
  ...
  var result = await saveLibraryItem({ name, unit, cost, markup, url: buyUrl, affiliate_id: (type === 'affiliate' ? null : null) });
  if (result) {
...
async function saveRowToLibrary(btn) {
  ...
  // Get affiliate info from the row
  var affKey = tr.getAttribute('data-affkey') || null;
  var buyUrl = null;
  var affiliateId = null;
  if (affKey && typeof affiliateLinksData !== 'undefined' && affiliateLinksData[affKey]) {
    buyUrl = affiliateLinksData[affKey].url || null;
    affiliateId = affiliateLinksData[affKey].id || null;
  }
  if (!buyUrl) {
  ...
  function showSuggestions(query) {
    ...
    matches.forEach(item => {
      ...
      li.innerHTML =
        ...
        '<span class="lib-ta-meta">' + _libEscHtml(item.source) + ' &bull; ' + _libEscHtml(item.unit) + ' &bull; $' + item.cost.toFixed(2) + '</span>';
